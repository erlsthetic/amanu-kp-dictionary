...
class _SearchScreenState extends State<SearchScreen> {
  ...
  late final controller = Get.put(SearchWordController(
      fromKulitan: widget.fromKulitanScanner, kulitanQuery: widget.input));
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (appController.isFirstTimeSearch) {
        showTutorial();
      }
    });
  }
    ...
    Future.delayed(Duration(seconds: 1), () {
      tutorialCoachMark = TutorialCoachMark(
          ...
          onFinish: () async {
            ...
            prefs.setBool("isFirstTimeSearch", false);
            appController.isFirstTimeSearch = false;
          },
          onSkip: () async {
            SharedPreferences prefs = await SharedPreferences.getInstance();
            prefs.setBool("isFirstTimeSearch", false);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Scaffold(
          key: appController.isFirstTimeSearch ? controller.searchKey : null,
          body: Stack(
            children: [
              Positioned(
                ...
                child: Obx(
                  () => Container(
                      key: appController.isFirstTimeSearch
                          ? controller.searchListKey
                          : null,
                      ...
                      child: controller.suggestionMap.length != 0
                          ? ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                String wordID = controller.suggestionMap.keys
                                    .elementAt(index);
                                List<String> type = [];
                                for (var meaning in controller
                                ...
                                return BrowseCard(
                                  ...
                                  wordId: wordID,
                                  word: controller.suggestionMap[wordID]
                                      ["word"],
                                  type: type,
                                  ...
                                  player: player,
                                  foundOn:
                                      controller.foundOn[wordID] ?? "engTrans",
                                );
                              },
                            )
